...
const ToDo = () => {
  ...
  const getTodo = () => {
    const todoId = 'unique_todo_id'; // Use the same ID you used in saveTodo
    ...
    firestore()
      ...
      .then(doc => {
        if (doc.exists) {
          ...
          setData([]); // If the document doesn't exist, set data to an empty array or handle it accordingly
        }
      })
      .catch(err => {
        console.log(err);
