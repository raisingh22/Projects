...
const ToDo = () => {
  ...
  const todoCollectionRef = firestore().collection('todo');
  useEffect(() => {
    fetchData();
  }, [isFocused]);
...
const handleAddTodo = async () => {
    try {
      // Add a new document to Firestore with the newTodo data
      const newTodoDocRef = await todoCollectionRef.add({
        todoText: newTodo,
        // Add other fields as needed
      });

      // Update the local state with the new todo
      setData([...data, { id: newTodoDocRef.id, todoText: newTodo }]);
      se(''); // Clear the input field
  ...
  const fetchData = async () => {
    try {
      const snapshot = await todoCollectionRef.get();
      const todoData = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
      setData(todoData);
    } catch (error) {
